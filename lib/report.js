...
function Report(author, dir, duration, branch) {
  this.author = author;
  this.dir = dir;
...
Report.prototype.run = function(done) {
  ...
  var cmd = 'git log --since ' + this.duration.toString() + '.hours --oneline --shortstat -C --author="'+ this.author + '" ' + this.branch + '..';
  ...
  cp.exec(cmd, {cwd: this.dir}, function(err, stdout, stderr) {
    ...
    function parseLine(line) {
      var filesChanged = line.match(/(\d*) file(s?) changed/),
          insertions = line.match(/(\d*) insertion(s?)/),
          deletions = line.match(/(\d*) deletion(s?)/);

      if(filesChanged) results.files += parseInt(filesChanged[1], 10);
      if(insertions) results.insertions += parseInt(insertions[1], 10);
      if(deletions) results.deletions += parseInt(deletions[1], 10);
    }
  });
};
